const WebInfo = {
  getData: async function () {
    ...
    await (async () => {
      try {
        data['general'] = {};
        data['general'].datetimeClient = new Date().toLocaleString('en-CA', {
          hour12: false,
        });
    ...
    await (async () => {
      try {
        ...
        data['device'].touchSupport =
          'ontouchstart' in window || window.TouchEvent ? 'yes' : 'no';
      } catch (e) {
        console.warn(e);
      }
    })();
    ...
    await (async () => {
      try {
        ...
        data['screen'].orientation =
          window.screen.width < window.screen.height ? 'portrait' : 'landscape';
      } catch (e) {
        console.warn(e);
      }
    })();
      ...
      try {
        return new Promise((resolve, reject) => {
          window.addEventListener('devicelight', function (event) {
            ...
            data['sensors'].brightness = event.value < 50 ? 'dark' : 'bright';
