...
const botUserAgents = fs
  .readFileSync(path.join(__dirname, '/server/COUNTER_Robots_list.txt'))
  .toString();
...
app.get(/\/.*?.*/, async function (req, res) {
  ...
  const url = req.originalUrl.replace(/\?.*$/g, '').replace(/^\/|\/$/g, '');
  if (urlList[url]) {
    if (urlList[url].count != 0) {
      for (let userAgent of botUserAgents.split('\n'))
        if (
          (userAgent &&
            new RegExp(userAgent).test(req.headers['user-agent'])) ||
      ...
      fs.readFile(path.join(__dirname, '/client/index.html'), (err, data) => {
        ...
        res.send(
          data
            .toString()
            .replace('<<<REDIRECT>>>', JSON.stringify(urlList[url].redirect))
        );
        res.end();
      });
...
app.post(/\/.*?.*/, async function (req, res) {
  ...
  const data = deepmerge(req.body, await WebInfo.getData(req));
  res.end('');
...
async function interactiveShell() {
  ...
  function listItems() {
    console.log('Items:');
    for (let url of Object.keys(urlList)) {
      const c = urlList[url].count;
      console.log(
        `    [${c}]${' '.repeat(
          12 - c.toString().length - url.toString().length
  ...
  async function editEntry(url, def) {
    ...
    urlList[url].redirect =
      (await rl.questionAsync('Redirection URL: ')) || def.redirect;
    urlList[url].count =
      (await rl.questionAsync('Maximum Usage: ')) || def.count;
    await saveUrlList(urlList);
  }
  while (true) {
    ...
    switch (command) {
      case 'help':
        ...
        break;
      case 'add':
        url = await rl.questionAsync('URL: ');
        if (!url) {
          url = '';
          for (let i = 0; i < 5; i++) url += Math.floor(Math.random() * 10);
        }
        await editEntry(url, {
          count: -1,
          redirect: '/',
      ...
      case 'rm':
        url = await rl.questionAsync('URL: ');
        confirmation = await rl.questionAsync(`Removing URL: ${url} [Y/N]: `);
      ...
      case 'clr':
        ...
        confirmation = await rl.questionAsync(`Clearing URL: ${url} [Y/N]: `);
        if (isYes(confirmation)) {
...
let server;
if (USE_HTTPS) {
  const p = '/etc/letsencrypt/live/emilien.ca/';
  const credentials = {
...
server
  ...
  .on('error', (e) => {
    ...
    console.log(
      `Server already listening on localhost:${port}. Interactive shell started.\n`
    );
    interactiveShell();
  });
